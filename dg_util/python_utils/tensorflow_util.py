...
def kernel_to_image(data, padsize=1, padval=0):
    ...
    data_shape = tuple(data.get_shape().as_list())
    data = tf.transpose(tf.reshape(data, ((n, n) + data_shape[1:])), ((0, 2, 1, 3) + tuple(range(4, ndim + 1))))
    data_shape = tuple(data.get_shape().as_list())
        ...
        for var_name, saved_var_name in var_names:
            ...
            if var_shape == saved_shapes[saved_var_name]:
                ...
                print(
                    ...
                    + str(var_shape)
                    + " = "
                    + str(int(np.prod(var_shape) * 4 / 10 ** 6))
                    + "MB"
                )
                restored_var_names.add(saved_var_name)
            else:
                print(
                    "Shape mismatch for var", saved_var_name, "expected", var_shape, "got", saved_shapes[saved_var_name]
                )
...
def l2_regularizer():
    with tf.variable_scope("l2_weight_penalty"):
        l2_weight_penalty = 0.0005 * tf.add_n([tf.nn.l2_loss(v) for v in tf.trainable_variables()])
...
class summary_writer(tf.summary.FileWriter):
    def __init__(self, path, graph):
        super(summary_writer, self).__init__(path, graph)
        import threading
    ...
    def add_summary(self, summary, global_step=None):
        ...
        if self.count < global_step:
            ...
            self.count += 1
        super(summary_writer, self).add_summary(summary, self.count)
        self.flush()
        self.lock.release()


def mean_rank(inputs, labels):
    ranks = np.argsort(-inputs, axis=1)
    ranks = np.argsort(ranks, axis=1)
    correct_ranks = ranks[np.arange(constants.BATCH_SIZE), labels]
    return (np.mean(correct_ranks) + 1).astype(np.float32)
